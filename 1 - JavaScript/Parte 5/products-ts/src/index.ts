...
const form = document.querySelector("#formProduct") as HTMLFormElement;
const imgPreview = document.querySelector("#imgPreview") as HTMLImageElement;
const tbody = document.querySelector("#tabla tbody") as HTMLTableSectionElement;
...
function showProduct(product: Product): void {
    ...
    btnDelete.append("X");
    btnDelete.dataset.id = String(product.id);
...
async function deleteProduct(e: Event): Promise<void> {
    const button = (e.target as HTMLButtonElement);
    try {
        await productosService.delete(+button.dataset.id!);
